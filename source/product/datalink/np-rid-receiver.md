...
- 串口输出数据协议可设置，支持GB 46750、MAVLink或JSON；
...
- 具备数据输出能力，可通过串口输出，支持GB 46750、MAVLink或JSON；
...
RID 设备出厂默认输出 JSON格式数据，连接RID模块USB至计算机，可直接通过串口助手查看无人机广播数据。
...
| CFG_PROTOCOL   | 数据输出协议格式<br />0：GB46750<br />1：MAVLink，HEARTBEAT、GPS_RAW_INT、OpenDroneID 相关消息<br />2：JSON（**默认**） |                                                              |
...
- GB46750 协议：也就是将收到的数据“透传”出来，详细请参考[国家标准|GB 46750-2025](https://openstd.samr.gov.cn/bzgk/std/newGbInfo?hcno=56E7666BF313B2F51A747F2BEEE4F896)，设置命令如下：
...
- JSON：默认，以字符串格式输出，可读性强；设置命令如下：

  ```bash
  param set CFG_PROTOCOL 2
  ```

> 注意：由于中国国标协议内容与 MAVLink 协议内容并不完全一致，故为了能够通过 MAVLink 输出，需要借用 MAVLink 消息字段进行传输，稍微不同的几个字段对应如下表：
>
> | 消息                           | 原始字段            | 映射 GB 后的含义  |
> | ------------------------------ | ------------------- | ----------------- |
