...
pnpm docs:build       # 生产构建，产物在 .vitepress/dist
...
采用 VitePress 默认布局，`srcDir` 指向 `source/`（内容位于 `source/` 下），中文为默认语言，英文放在 `source/en/` 子目录下（当前暂无英文内容）：
...
| 资料下载 | 待定 | `/download/` | 固件、软件、文档等资料下载 | 待建 |
...
- `/product/`属于`product`类，产品列表页采用卡片方式展示`product`目录下的产品
- 目录结构为`product/<xx-category>/<yy-detail.md>`, `<xx-category>` 为产品分类，`<yy-detail.md>` 为产品详情页，`xx`和`yy` 是排序前缀，每个产品一个 Markdown 文件
- 卡片展示依赖 `frontmatter（title / cover / summary / price / tags）`等；**可选字段未填写时，卡片上不显示对应内容**（如无 `cover` 就不渲染封面图、无 `price` 就不显示价格）
- 支持按子目录进行分类筛选（不使用 `tags` 做分类筛选）和产品全文搜索；如果展示的产品太多则需要分页
- 子目录映射为产品分类，分类名优先从 `index.md` 的 frontmatter `title` 提取，其次文件夹名去除排序前缀
- 产品列表数据必须排除所有路径中的 `index.md`，避免分类首页被当成产品卡片
- md文件映射产品，产品名优先采用 frontmatter `title`，其次文件名去除排序前缀
- 路由优先使用 frontmatter `permalink`，否则回退文件路径

### 用户手册（manual）和开发指南（develop）

- `/manual/`和`/develop/`属于 `docs`类，需要在左侧自动生成侧边栏 `sideBar`，右侧显示`TOC`
- 目录结构为`manual/<xx-group>/<yy-detail.md>`，`<xx-group>` 是分组目录，`<yy-detail.md>` 是具体页面，`xx`和`yy` 是排序前缀
- 侧边栏在**配置加载时**，分别读取 `manual/`和`develop/` 目录树生成（`docsSidebar()` 返回静态 `SidebarItem[]`，非运行时函数）
- 子目录映射为分组，分组名优先从 `index.md` 的 frontmatter `title` 中提取，其次从文件夹名去除排序前缀
- 文档侧边栏不显示任何 `index.md`；`index.md` 只用于分组标题和分组首页
...
- 目录结构为`news/<xx-category>/<yyyymmdd-detail.md>`（blog 同理），`<xx-category>` 是分类目录，`<yyyymmdd-detail.md>` 是具体页面，`xx` 是分类排序前缀，`yyyymmdd` 是文章排序前缀（文章新建日期）
- 列表页展示的条目，应该包含图片、标题、简介、时间、作者等信息，可以从 `frontmatter（title / cover / summary / tags / date / author）`获取
- 子目录映射为分类，分类名优先从 `index.md` 的 frontmatter `title` 提取，其次文件夹名去除排序前缀
- md 映射为条目，条目名优先 frontmatter `title`，否则文件名去排序前缀
- 博客和新闻列表数据必须排除所有路径中的 `index.md`
- 路由优先使用 frontmatter `permalink`，否则回退文件路径
...
- **待建项目**：暂只保留目录占位，不接入 nav，内容模型待定

## 内容约定

### 文件名规范

文件名和路径名，只允许数字、小写字母和横杠，不允许中文、下划线、标点等。
...
产品分类和文章分类只由目录决定，`tags` 不用于产品分类筛选。`Frontmatter` 并不需必须的，其中 `permalink` 用于固定 URL（如迁移自旧站、需要保持原链接时）；链接写以 `/` 开头的站内相对路径，`base` 由 VitePress 自动拼接，不要手写域名。多数页面可省略、由文件路径自动生成。

`draft: true` 在开发环境允许预览，但生产构建、产品/文章列表和本地搜索索引必须排除草稿；未填写 `draft` 时按正式内容处理。重复的 `permalink` 不阻断构建，但必须输出构建警告并列出冲突文件。
...
- 提交前至少运行 `pnpm docs:build`；同时检查 Markdown 文件名、Frontmatter、站内链接和图片路径。构建产物 `.vitepress/dist` 不提交，生产发布目录为 `.vitepress/dist`，`public/CNAME` 随构建复制
